/* L'Oracle — rendu de la page (vanilla JS, sans dépendance).
   Lit data/predictions.json, calcule score + calibration, affiche tout. */

// Renseigné automatiquement une fois le dépôt public créé.
const REPO_URL = "https://github.com/Jordan-Bourillot/oracle-triskell";

const CAT_LABEL = { crypto: "Crypto", meteo: "Météo" };
...
// Date "AAAA-MM-JJ" -> "27 juin 2026", sans piège de fuseau horaire.
...
async function init() {
  ...
  try {
    ...
    document.body.insertAdjacentHTML(
      "beforeend",
      '<p style="text-align:center;color:#a63a2e;font-family:monospace">Impossible de charger les prédictions.</p>',
    );
    return;
  }
  ...
  if (data.updated_at) {
    $("#updated").textContent = "Dernière mise à jour : " + fmtDateTime(data.updated_at);
  }
  renderRepo();
  renderScore();
  renderCalibration();
...
function renderScore() {
  ...
  const total = STATE.preds.length;
  ...
  if (res.length) {
    const brier =
      res.reduce((acc, p) => acc + (p.probability / 100 - p.result.hit) ** 2, 0) / res.length;
...
function renderCalibration() {
  ...
  if (!bins.length) {
    plot.innerHTML = drawCalibSvg([]); // diagramme vide avec la diagonale
    legend.innerHTML =
  ...
  legend.innerHTML =
    ...
    bins
      .map(
        (b) =>
          `<div class="lg-row"><span>${b.lo}–${b.hi} %</span><span>${Math.round(
            b.freq * 100,
          )} % (${b.n})</span></div>`,
      )
      .join("");
...
function drawCalibSvg(bins) {
  ...
  const y = (f) => mT + (1 - f) * iH;
  const ink = "#1c1b19", rule = "#ddd5c4", accent = "#2e2a6b", muted = "#6b6557";

  let s = `<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Courbe de calibration">`;
  // grille
  for (const g of [0, 25, 50, 75, 100]) {
    s += `<line x1="${x(g)}" y1="${mT}" x2="${x(g)}" y2="${mT + iH}" stroke="${rule}" stroke-width="1"/>`;
    s += `<line x1="${mL}" y1="${y(g / 100)}" x2="${mL + iW}" y2="${y(g / 100)}" stroke="${rule}" stroke-width="1"/>`;
    s += `<text x="${x(g)}" y="${mT + iH + 24}" fill="${muted}" font-family="monospace" font-size="11" text-anchor="middle">${g}</text>`;
    s += `<text x="${mL - 10}" y="${y(g / 100) + 4}" fill="${muted}" font-family="monospace" font-size="11" text-anchor="end">${g}</text>`;
  }
  // cadre
  s += `<rect x="${mL}" y="${mT}" width="${iW}" height="${iH}" fill="none" stroke="${ink}" stroke-width="1.5"/>`;
  // diagonale parfaite
  s += `<line x1="${x(0)}" y1="${y(0)}" x2="${x(100)}" y2="${y(1)}" stroke="${accent}" stroke-width="1.5" stroke-dasharray="5 4"/>`;
  s += `<text x="${x(100) - 6}" y="${y(1) + 16}" fill="${accent}" font-family="monospace" font-size="10" text-anchor="end">calibration parfaite</text>`;
  // axes (titres)
  s += `<text x="${mL + iW / 2}" y="${H - 4}" fill="${ink}" font-family="monospace" font-size="11" text-anchor="middle">probabilité annoncée (%)</text>`;
  s += `<text x="14" y="${mT + iH / 2}" fill="${ink}" font-family="monospace" font-size="11" text-anchor="middle" transform="rotate(-90 14 ${mT + iH / 2})">part réalisée (%)</text>`;

  if (bins.length) {
    // ligne reliant les points (triés)
    const pts = [...bins].sort((a, b) => a.meanP - b.meanP);
    s += `<polyline fill="none" stroke="${ink}" stroke-width="2" points="${pts
      .map((b) => `${x(b.meanP)},${y(b.freq)}`)
      .join(" ")}"/>`;
    for (const b of pts) {
      const r = 5 + Math.min(14, Math.sqrt(b.n) * 4);
      s += `<circle cx="${x(b.meanP)}" cy="${y(b.freq)}" r="${r}" fill="${accent}" fill-opacity="0.18" stroke="${accent}" stroke-width="1.5"><title>${b.lo}–${b.hi} % : réalisé ${Math.round(
        b.freq * 100,
      )} % sur ${b.n}</title></circle>`;
...
function renderFilters() {
  ...
  const labels = { all: "Tout", crypto: "Crypto", meteo: "Météo" };
  const box = $("#filters");
  box.innerHTML = cats
    .map(
      (c) =>
        `<button class="filter-btn" data-cat="${c}" aria-pressed="${c === STATE.filter}">${
          labels[c] || c
        }</button>`,
    )
    .join("");
  box.querySelectorAll(".filter-btn").forEach((btn) =>
    btn.addEventListener("click", () => {
      ...
      box.querySelectorAll(".filter-btn").forEach((b) =>
        b.setAttribute("aria-pressed", b.dataset.cat === STATE.filter),
      );
      renderPending();
    }),
  );
...
function buildCard(p) {
  ...
  $(".lock", card).innerHTML =
    `🔒 <b>Figée le ${fmtDateTime(p.created_at)}</b> · empreinte ${p.hash || "—"}`;
  ...
  if (p.result) {
    ...
    const proof = p.result.evidence_url
      ? ` · <a href="${p.result.evidence_url}" target="_blank" rel="noopener">preuve</a>`
      : "";
    meta.insertAdjacentHTML(
      "beforeend",
      `<div class="row result-line"><span class="k">Résultat réel</span><span>${p.result.actual_label}${proof}</span></div>`,
    );
  }
  return card;
}
...
function renderHistory() {
  ...
  list.innerHTML = "";
  const items = resolved().sort(
    (a, b) => new Date(b.result.resolved_at) - new Date(a.result.resolved_at),
  );
  if (!items.length) {
    list.innerHTML =
      '<p class="empty-note">Rien n\'a encore été tranché. Les premiers verdicts arriveront à la première échéance.</p>';
    return;
  }
  for (const p of items) list.appendChild(buildCard(p));
}
...
function renderRepo() {
  ...
  if (REPO_URL && !REPO_URL.includes("__")) {
    line.innerHTML =
      `Code et historique publics : <a href="${REPO_URL}" target="_blank" rel="noopener">${REPO_URL.replace(
        "https://",
        "",
      )}</a> · vérifiable avec <code>npm run verify</code>.`;
  } else {
